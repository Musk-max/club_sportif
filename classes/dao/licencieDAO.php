...
public function create(licencieModel $licencie){
    try{
        $submit = $this->licenciePdo->prepare("INSERT INTO licencies(nom,prenom,contact_id)VALUES(?, ?, ?)");

        $submit->execute([$licencie->getNom(),$licencie->getPrenom(),$licencie->getContact_id()]);
...
public function update(licencieModel $licencie){
    try{
        $submit = $this->licenciePdo->prepare ("UPDATE licencies SET nom=?, prenom=?, contact_id=? WHERE licencie_id = ?");
        $submit->execute ([$licencie->getNom(), $licencie->getPrenom(), $licencie->getContact_id(), $licencie->getLicencie_id()]);
