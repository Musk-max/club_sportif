<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../../css/styles.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootswatch@4.5.2/dist/united/bootstrap.min.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootswatch@4.5.2/dist/united/variables.css">
    <title>supprimer contact</title>
</head>
<body>
    <?php include("../../partials/_header.php");?>
    <h1> supprimer contact</h1>
    <a href = "HomeController.php">Retour à la liste des contacts</a>
    <?php if ($contact): ?>
        <p> voulez-vous vraiment supprimer le contact? "<?php echo $contact -> getNom(); ?> <?php echo $contact -> getPrenom(); ?> <?php echo $contact -> getEmail(); ?> <?php echo $contact -> getNumero(); ?>" ? </p>
        <form action = "DeleteContactController.php? contact_id = <?php echo $contact -> getContact_id(); ?>" method = "post">
            <input type = "submit" name = "action" value = "oui supprimer"> 
        </form> 
    <?php else: ?>
        <p>le contact n'a pas été trouvé</p>
    <?php endif; ?>
</body>
</html>